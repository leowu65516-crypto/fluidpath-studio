...
| `src/__tests__/` | 36 个测试文件 / 240 用例，见 §6 |
...
npx tsc --noEmit                 # 类型检查
npx vitest run                   # 全量测试（当前 240 个，36 文件）
...
*文档结束。接手后建议第一步：`npx vitest run` 确认 240 绿，再读 `src/geometry.ts` 的 `computeDisabledPipes`（相位 BFS + 压域）与 `computeDemandPipes`（需求域 per-root BFS）。*
