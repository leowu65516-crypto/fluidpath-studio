...
export function ValidationPanel({ onClose }: { onClose: () => void }) {
  ...
  const addSelected = (expected: "flow" | "stop") => {
    if (!selectedPipeIds.length) return toast("先在画布上选中要验收的管路", "error");
  ...
  const save = () => {
    if (!name.trim()) return toast("请填写验收工况名称", "error");
    if (!mustFlow.length && !mustStop.length) return toast("至少指定一条必须流动或停止的管路", "error");
  ...
  const chips = (expected: "flow" | "stop", ids: string[]) => (
    <div className={`validation-chips ${expected}`}>
      {ids.map((id) => <button key={id} className="validation-chip" title="定位管路" onClick={() => focusElement(id)}>{pipeName.get(id) ?? id}<span onClick={(e) => { e.stopPropagation(); remove(expected, id); }}>×</span></button>)}
    </div>
  );

  return (
    <aside className="validation-panel" data-ui="1">
      <div className="validation-head"><h2>✓ 工况验收</h2><button className="advice-close" onClick={onClose}>✕</button></div>
      <p className="validation-help">记录当前泵阀状态，并指定哪些管路必须流动或停流。验证在副本中运行，不会改动当前画布。</p>

      <div className="validation-create">
        <input value={name} placeholder="验收名称，例如：断水停泵" onChange={(e) => setName(e.target.value)} />
        <div className="validation-buttons">
          <button className="btn" onClick={() => addSelected("flow")}>＋ 选中管设为应流</button>
          <button className="btn" onClick={() => addSelected("stop")}>＋ 选中管设为应停</button>
        </div>
        <div className="validation-expect"><b>必须流动</b>{chips("flow", mustFlow)}</div>
        <div className="validation-expect"><b>必须停流</b>{chips("stop", mustStop)}</div>
        <button className="cond-save-btn" onClick={save}>💾 保存当前工况为验收</button>
      </div>

      <div className="validation-list-head"><b>已定义验收工况（{cases.length}）</b><button className="btn" disabled={!cases.length} onClick={run}>▶ 运行全部</button></div>
      {!cases.length && <div className="cond-empty">新图纸没有验收标准。先摆好泵阀状态，选中关键管路，再建立案例。</div>}
      <div className="validation-list">
        {cases.map((c) => {
          ...
          return <div className={`validation-case${result ? result.passed ? " pass" : " fail" : ""}`} key={c.id}>
            ...
            <button className="btn ghost sq" title="删除" onClick={() => deleteValidationCase(c.id)}>×</button>
